...
<table class="table table-bordered">
    ...
    <tbody>
        @foreach ($carts as $cart)
            <tr id="cart-row-{{ $cart->cart_id }}">
                <td>{{ $cart->cart_id }}</td>
                <td>{{ $cart->uuid }}</td>
                <td>{{ $cart->status }}</td>
                <td>${{ number_format($cart->subtotal_price, 2) }}</td>
                ...
                <td>
                    <div class="btn-sty" style="display:flex; justify-content:space-around; gap:1rem; text-decoration:none;">
                        <button type="button" class="btn btn-info" onclick="toggleItems('{{ $cart->cart_id }}')">Show Items</button>
                    <a href="{{ route('carts.edit', $cart->cart_id) }}"><button type="button" class="btn btn-primary">edit</button></a>
            ...
            <tr id="cart-items-{{ $cart->cart_id }}" style="display: none;">
                <td colspan="6">
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @foreach ($cart->items as $item)
                                <tr>
                                    ...
                                    <td><img src="{{ asset('baackend_images/' . $item->product_image) }}" alt="{{ $item->product_name }}" width="50"></td>
