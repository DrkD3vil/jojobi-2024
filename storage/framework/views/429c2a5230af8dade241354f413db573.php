...
<?php $__env->startSection('content'); ?>
    ...
    <!-- Add Product Form -->
<form action="<?php echo e(route('pos.addCartItem', $cart->cart_id)); ?>" method="POST" class="mb-3">
    <?php echo csrf_field(); ?>
...
<table class="table table-bordered">
    ...
    <tbody id="cartBody">
        ...
        <?php $__empty_1 = true; $__currentLoopData = $cart->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            ...
            <tr id="cartItem-<?php echo e($item->id); ?>">
                ...
                <td>
                    <img src="<?php echo e(asset('baackend_images/' . $item->product_image)); ?>" alt="Product Image" width="50">
                </td>
                <td><?php echo e($item->product_name); ?></td>
                <td>
                    <input type="number" class="form-control form-control-sm" value="<?php echo e($item->quantity); ?>" min="1" id="quantity-<?php echo e($item->id); ?>" data-id="<?php echo e($item->id); ?>" onchange="updateQuantity(<?php echo e($item->id); ?>)">
                </td>
                <td><?php echo e($item->category_name ?? 'No Category'); ?></td>
                <td>$<?php echo e(number_format($item->price, 2)); ?></td>
                <td>
                    <button type="button" class="btn btn-danger btn-sm remove-cart-item" data-id="<?php echo e($item->id); ?>" onclick="removeItem(<?php echo e($item->id); ?>)">Remove</button>
                </td>
            </tr>
    ...
    <tfoot>
        <tr>
            ...
            <td colspan="2" id="subtotal"><strong>$<?php echo e(number_format($subtotal_price, 2)); ?></strong></td>
        </tr>
    </tfoot>
</table>

    <!-- Proceed Button -->
    <form action="<?php echo e(route('pos.proceedCart', $cart->cart_id)); ?>" method="POST" class="text-end">
        <?php echo csrf_field(); ?>
...
<script>
        ...
        $('#barcode').on('input', function () {
            ...
            if (query.length > 0) {
                $.ajax({
                    url: "<?php echo e(route('pos.products.search')); ?>",
                    type: "GET",
                    data: { query: query },
        ...
        $(document).on('click', '.add-to-cart', function () {
            ...
            $.ajax({
                url: '<?php echo e(route('pos.addCartItem', $cart->cart_id)); ?>',
                method: 'POST',
                data: {
...
function updateQuantity(id) {
    ...
    if (quantity && quantity > 0) {
        $.ajax({
            url: '<?php echo e(route("pos.updateQuantity")); ?>',  // Route to update the cart
            method: 'POST',
            data: {
                _token: '<?php echo e(csrf_token()); ?>',
                id: id,
...
function removeItem(id) {
    $.ajax({
        url: '<?php echo e(route('pos.removeCartItem')); ?>', // Route to remove the item
        method: 'POST',
        data: {
            _token: '<?php echo e(csrf_token()); ?>',
            id: id
...
<?php echo $__env->make('adminBackend.adminLayout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/bijoydey/Desktop/jojobi/resources/views/adminBackend/pos/edit.blade.php ENDPATH**/ ?>